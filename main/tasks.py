...
# celery beat pracice task without the functionality og logging data to the database, rather simple tasks being used for returning output in the browser and the celery worker terminal
...
@shared_task
def cache_timestamp_task():
    now = datetime.utcnow().isoformat()
    cache.set('last_beat_run', now, timeout=150)
    print(f'the cache is updated with the timestamp: {now}')
    
@shared_task
def clear_task():
    cache.delete('task_list')
    print('cache_list is cleared by the celery beat')